            ...
            if (value != null && value?.traveler_list.Count > 0)
            {
                try
                {
                    ...
                    using (TOPEBizCreateTripEntities context = new TOPEBizCreateTripEntities())
                    {
                        ...
                        sql = @"  select distinct dta_travel_empid as emp_id, dta_appr_empid as appr_id, dta_type as appr_type
                        ...
                        var dataApporver_Def = context.TravelerApproverConditionModelList.FromSqlRaw(sql, parameters.ToArray()).ToList();
                        ...
                        parameters = new List<OracleParameter>();
                        parameters.Add(context.ConvertTypeParameter("doc_no", doc_no, "char"));
                        var dataApporver_Def2 = context.TravelerApproverConditionModelList.FromSqlRaw(sql, parameters.ToArray()).ToList();
                        ...
                        sql = @$"  select distinct dta_travel_empid as emp_id, dta_appr_empid as appr_id, dta_type as appr_type
                                ...
                                and a.dh_code = :doc_no  {sql_emp_list}  order by a.dta_travel_empid, a.dta_appr_level";
                        ...
                        var dataApporver_not_active = context.TravelerApproverConditionModelList.FromSqlRaw(sql, parameters.ToArray()).ToList();
                        ...
                        var dataApporver_in_active = context.TravelerApproverConditionModelList.FromSqlRaw(sql, parameters.ToArray()).ToList();


                        sql = @$" select distinct a.dta_travel_empid as emp_id, to_char((max(nvl(a.dta_appr_level,0))+1)) as approve_level
                                 from BZ_DOC_TRAVELER_APPROVER a 
                                 where  dta_doc_status is not null and a.dta_type ='2' 
                                 and a.dh_code = :doc_no  {sql_emp_list} group by a.dta_travel_empid";
                        ...
                        var dataMaxApprover_Level = context.TravelerApproverConditionModelList.FromSqlRaw(sql, parameters.ToArray()).ToList();


                        sql = $"SELECT * FROM BZ_DOC_HEAD WHERE DH_CODE = :doc_no ";
                        parameters = new List<OracleParameter>();
                        parameters.Add(context.ConvertTypeParameter("doc_no", doc_no, "char"));
                        var dataDoc = context.TravelerDocHeadList.FromSqlRaw(sql, parameters.ToArray()).FirstOrDefault();

                        sql = "SELECT EMPLOYEEID, ENTITLE, ENFIRSTNAME, ENLASTNAME, ORGNAME, MANAGER_EMPID, SH, VP, AEP, EVP, SEVP, CEO FROM BZ_USERS";
                        parameters = new List<OracleParameter>();
                        ...
                        sql = @"SELECT distinct to_char(a.DTE_EMP_ID) as EMPLOYEEID, null as ENTITLE, null as ENFIRSTNAME, null as ENLASTNAME, null as ORGNAME, null as MANAGER_EMPID, b.SH, b.VP, b.AEP, b.EVP, b.SEVP, b.CEO
                             ,b.COST_CENTER
                        ...
                        sql = "SELECT * FROM BZ_APPROVER_CONDITION WHERE upper(doc_type) like upper(:doc_head_type || '%') order by budget_limit ";
                        parameters = new List<OracleParameter>();
                        ...
                        parameters = new List<OracleParameter>();
                        var positionList = context.TravelerUsersModelList.FromSqlRaw(sql, parameters.ToArray()).ToList();
                        ...
                        if (dataDoc != null)
                        {
                            ...
                            if (dataDoc.DH_TYPE.ToUpper() == "OVERSEA")
                            {
                                ...
                                sql = "SELECT * FROM BZ_DOC_TRAVELER_EXPENSE WHERE dte_status = 1 and DH_CODE = :doc_no ";
                                ...
                                foreach (var item in apprListCAP)
                                {
                                    ...
                                    if (travelExpen != null)
                                    {
                                        ...
                                        sql = "SELECT * FROM BZ_MASTER_COSTCENTER_ORG WHERE COST_CENTER = :item_cost_center "; //TODO COST CENTER ID
                                        parameters = new List<OracleParameter>();
                                        ...
                                        if (masterCostCenterList.Count == 0 && item.appr_position == "SEVP")
                                        {
                                            ...
                                            sql = "select null as SEVP, employeeid as CEO from  bz_users a  where  function = 'MGT' and department is null and sections is null ";
                                            parameters = new List<OracleParameter>(); ;
