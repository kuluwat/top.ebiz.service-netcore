                ...
                using (var connection = context.Database.GetDbConnection())
                {
                    ...
                    if (data != null)
                    {
                        ...
                        foreach (var item in data)
                        {
                            ...
                            // Use FromSqlRaw with parameterized queries to prevent SQL injection
        ...
        public DocDetailModel searchDetail(DocDetailSearchModel value)
        {
            ...
            try
            {
                using (TOPEBizEntities context = new TOPEBizEntities())
                {
                    ...
                    docHeadStatus = [.. context.DocHeadModels.FromSqlRaw("select to_char(dh_doc_status) as document_status from bz_doc_head h where h.dh_code ={0}", id_doc)];
                    ...
                    " b.TS_NAME document_status, a.DH_TYPE_FLOW from BZ_DOC_HEAD a left join BZ_MASTER_STATUS b on a.DH_DOC_STATUS=b.TS_ID WHERE DH_CODE = {0}", id_doc).ToList();
                    if (docCheckTab != null) { }
                    ...
                    if (traveler != null)
                    {
                        ...
                        var dataCheck_Def = context.TravelerApproverConditionModels
                            ...
                            " WHERE dta_action_status > 2 AND a.dta_type = 2 AND dh_code = :id_code", context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        ...
                        // Prepare the main query with placeholders for parameters
                        sql = "SELECT dta_appr_level, dta_travel_empid AS emp_id, (a.dta_action_status) AS approve_status, a.dta_appr_remark AS approve_remark, " +
                            " TO_CHAR(NVL(a.dta_appr_status,'true')) AS approve_opt FROM BZ_DOC_TRAVELER_APPROVER a WHERE dh_code = :id_code ";
                        ...
                        var dataApprover_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        var dataApproverLine_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        var dataApproverLine2_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        var dataApproverLine3_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        var dataApproverCAP_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        var dataApproverCAP2_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        var dataApproverCAP3_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        var dataApproverRevise_Def = context.TravelerApproverConditionModels.FromSqlRaw(sql, context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        ...
                        if (pf_doc_id == "3" || pf_doc_id == "4" || pf_doc_id == "5")
                        {
                            ...
                            dataApproverLine2_Def = context.TravelerApproverConditionModels.FromSqlRaw(" select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id , " +
                                ...
                                "where ((dte_appr_opt = 'false' and dte_status = 1) or dte_appr_status = 30 ) and dh_code = :id_code ",
                                context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                            ...
                            dataApproverLine3_Def = context.TravelerApproverConditionModels.FromSqlRaw(" select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, '2' as approve_status, dte_appr_remark as approve_remark " +
                                " from BZ_DOC_TRAVELER_EXPENSE a where dte_status = 1 and dte_appr_status = 31 and dh_code = :id_code ",
                                context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                            ...
                            dataApproverCAP_Def = context.TravelerApproverConditionModels.FromSqlRaw(" select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, '3' as approve_status, dte_cap_appr_remark as approve_remark " +
                                " from BZ_DOC_TRAVELER_EXPENSE a where nvl(dte_cap_appr_status, 41) = '42' and(dte_cap_appr_opt = 'true' " +
                                " and dte_appr_opt = 'true') and dte_status = 1 and dh_code = :id_code ",
                                context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                            ...
                            dataApproverCAP2_Def = context.TravelerApproverConditionModels.FromSqlRaw("select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, '5' as approve_status, dte_cap_appr_remark as approve_remark " +
                                " from BZ_DOC_TRAVELER_EXPENSE a where nvl(dte_cap_appr_status, 41) = '42' and((dte_cap_appr_opt = 'false' and dte_status = 1) or(dte_appr_opt = 'false' and dte_appr_status = 32) or dte_cap_appr_status = 40) and dh_code = :id_code " +
                                " union select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, '5' as approve_status, dte_appr_remark as approve_remarkfrom BZ_DOC_TRAVELER_EXPENSE a " +
                                " where dte_status = 1 and ( dte_cap_appr_status = 23 and a.dte_cap_appr_opt = 'false') and dh_code = :id_code ",
                                context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                            ...
                            dataApproverCAP3_Def = context.TravelerApproverConditionModels.FromSqlRaw(" select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, " +
                                " '2' as approve_status, dte_cap_appr_remark as approve_remark from BZ_DOC_TRAVELER_EXPENSE a " +
                                "where dte_status = 1 and(dte_cap_appr_status = 41 or (dte_cap_appr_status is null and dte_appr_status = 32 " +
                                "and dte_appr_opt = 'true')) and dh_code = :id_code ",
                                context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                            ...
                            dataApproverRevise_Def = context.TravelerApproverConditionModels.FromSqlRaw(" select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, " +
                                ...
                                " where dte_status = 1 and(dte_appr_status = 23 or dte_cap_appr_status = 23) and dh_code = :id_code ",
                                context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                ...
                using (TOPEBizEntities context = new TOPEBizEntities())
                {
                    ...
                    if (pf_doc_id == "3" || pf_doc_id == "4" || pf_doc_id == "5")
                    {
                        ...
                        dataApproverLine2_Def = context.TravelerApproverConditionModels.FromSqlRaw(" SELECT TO_CHAR(dte_token) AS traveler_ref_id, dte_emp_id AS emp_id, " +
                            ...
                            " AND(dte_appr_status = 23 AND a.dte_appr_opt = 'false') AND dh_code = :id_code ",
                            context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        //line pendding
                        //sql = @" select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id
                        //                 , '2' as approve_status, dte_appr_remark as approve_remark
                        //                 from BZ_DOC_TRAVELER_EXPENSE a 
                        //                 where dte_status = 1 and dte_appr_status = 31 and dh_code = '" + value.id_doc + "' ";
                        //dataApproverLine3_Def = context.Database.SqlQuery<TravelerApproverConditionModel>(sql).ToList();

                        dataApproverLine3_Def = context.TravelerApproverConditionModels.FromSqlRaw(" select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, '2' as approve_status, dte_appr_remark as approve_remark" +
                            " from BZ_DOC_TRAVELER_EXPENSE a where dte_status = 1 and dte_appr_status = 31 " +
                            " and dh_code = :id_code", context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        ...
                        dataApproverCAP2_Def = context.TravelerApproverConditionModels.FromSqlRaw(" SELECT TO_CHAR(dte_token) AS traveler_ref_id, dte_emp_id AS emp_id, " +
                            ...
                            " OR(dte_appr_opt = 'false' AND dte_appr_status = 32) OR dte_cap_appr_status = 40) AND dh_code = :id_code " +
                            ...
                            " FROM  BZ_DOC_TRAVELER_EXPENSE a WHERE dte_status = 1 AND(dte_cap_appr_status = 23 AND a.dte_cap_appr_opt = 'false') AND dh_code = :id_code",
                            context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        ...
                        dataApproverCAP3_Def = context.TravelerApproverConditionModels.FromSqlRaw("select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id, " +
                            ...
                            " or (dte_cap_appr_status is null and  dte_appr_status = 32 and dte_appr_opt = 'true' ) ) and dh_code = :id_code ",
                            context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
                        ...
                        dataApproverRevise_Def = context.TravelerApproverConditionModels.FromSqlRaw("select to_char(dte_token) as traveler_ref_id, dte_emp_id as emp_id , " +
                            ...
                            " or dte_cap_appr_status = 23) and dh_code = :id_code", context.ConvertTypeParameter("id_doc", id_doc, "char")).ToList();
            ...
            try
            {
                using (TOPEBizEntities context = new TOPEBizEntities())
                {
                    ...
                    if (value.id_doc.IndexOf("T") > -1)
                    {
                        ...
                        var adminlist = context.SearchUserModels.FromSqlRaw("select emp_id as user_id from bz_data_manage where (pmsv_admin = 'true' or pmdv_admin = 'true') and emp_id = {0}", user_id).ToList();
                        if (adminlist != null)
                        {
                    ...
                    if (user_role != "1")
                    {
                        ...
                        emp_type = context.SearchUserModels.FromSqlRaw("SELECT TO_CHAR(t.user_type) AS user_type FROM (" +
                            ...
                            "AND t.doc_id = {0} AND t.emp_id = {1} ORDER BY user_type DESC", id_doc, user_id).ToList();
        ...
        public DocDetail3OutModel searchDetail4(DocDetail3Model value)
        {
            ...
            try
            {
                using (TOPEBizEntities context = new TOPEBizEntities())
                {
                    ...
                    #region ตรวจสอบสถานะใบงาน
                    var docHeadStatus = new List<DocHeadModels>();
                    sql = " select to_char(dh_doc_status) as document_status from bz_doc_head h where h.dh_code = '" + value.id_doc + "' ";
                    //docHeadStatus = context.Database.SqlQuery<DocHeadModel>(sql).ToList();
