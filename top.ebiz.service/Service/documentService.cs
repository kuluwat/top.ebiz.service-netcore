...
namespace top.ebiz.service.Service
{
    public class documentService
    {
        ...
        private Boolean AllApproveCAPApprover(string doc_id, ref string ret_doc_status)
        {
            Boolean ret = false;
            try
            {
        ...
        private Boolean AllApproveLineApprover(string doc_id, ref string ret_doc_status)
        {
            Boolean ret = false;
            var sql_query = "";
            try
                        ...
                        try
                        {
                            ...
                            if (value.docfile.Count > 0)
                            {
                                try
                                {
                                    ...
                                    System.IO.DirectoryInfo di = System.IO.Directory.CreateDirectory(_path_temp);
                                    //ลบจริงตอน save
                                    if (System.IO.Directory.Exists(_path_temp))
                                    {
                                    ...
                                    if (bcopyfile == true)
                                    {
                                        ...
                                        System.IO.DirectoryInfo difile = System.IO.Directory.CreateDirectory(_path_file);
                                        if (System.IO.Directory.Exists(_path_file))
                                        {
                                            //all files and folders in a directory 
                                            foreach (System.IO.FileInfo file in difile.GetFiles())
                                            {
                                        ...
                                        System.IO.DirectoryInfo ditemp = System.IO.Directory.CreateDirectory(_path_temp);
                                        //ใส่ try เนื่องจาก code ใช้งานได้ไม่ติดปัญหาแต่จะมี กรณีที่มีการลบ file ทั้งหมดออก folder temp ตาม doc นั้น  
                                        System.IO.Directory.CreateDirectory(_path_file);
                                        //ลบจริงตอน save
                                        if (System.IO.Directory.Exists(_path_temp))
                                        {
                                            //all files and folders in a directory 
                                            foreach (System.IO.FileInfo file in ditemp.GetFiles())
                                            {
                                                // copy to fodel file    
        ...
        private Boolean checkExpenUpdate(docFlow2_travel data1, docFlow2_travel data2)
        {
            Boolean ret = false;
            try
            {
        ...
        private Boolean Flow1Mail(decimal? doc_status, string doc_no)
        {
            Boolean ret = false;
            try
            {
        ...
        public DocFileListModel uploadfile()
        {
            ...
            try
            {
                ...
                try
                {
                    System.IO.DirectoryInfo di = System.IO.Directory.CreateDirectory(_Path);
